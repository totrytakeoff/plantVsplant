...
public:
	SunBullet(){
		...
		explode_render_offset.x = (frame_idle->getwidth() - frame_explode->getwidth()) / 2.0f;///爆炸位置偏移
	...
	void on_update(int delta) {
		...
		if (!valid){
			...
			animation_idle.on_update(delta);//sun 默认动画,不加没啥影响，但是错写成了explode会调用callback-> make sun can_remove 
		}
