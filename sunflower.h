...
public:
	Sunflower() {
		...
		animation_sun_text.set_atlas(&atlas_sun_text);	///我靠！ 忘记给animation_sun_text.set_atlas了。。。。。
	...
	void on_draw(const Camera& camera) {//绘制
		...
		}//tmd  这玩意怎么访问不了？？？这bug找我半天，是不是生命周期的问题？？\
		///我靠！ 忘记给animation_sun_text.set_atlas了。。。。。
	...
	void on_attack()
	{
		...
		bullet->set_velocity(is_facing_right ? velocity_sun.x : -velocity_sun.x,      velocity_sun.y);//设置子弹速度
